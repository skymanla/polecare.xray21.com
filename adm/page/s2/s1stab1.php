...
if(!empty($searchKey)){
	switch($searchType){
		case "chartnum":
			$searchQuery = " and chartnum like '%".$searchKey."%' ";
			$checked = "chartnum";
			break;
		case "name":
			...
			$checked = "name";
			break;
		case "patient_bandtype":
			$searchQuery = " and patient_bandtype in 
								(select seq, equiment_name from tbl_equiment where equiment_name like '%".$searchKey."%') ";
			$checked = "patient_bandtype";
			break;
	}
}
$count_sql = "select count(*) as cnt from $tbl where status='퇴원' and delete_flag != 1 ".$searchQuery;
...
$sql = "select *, (select equiment_name from tbl_equiment where seq=patient_bandtype) as bandName from 
		$tbl where status='퇴원' and delete_flag != 1 $searchQuery
		order by seq desc limit $show_offset_num, $limit_num";
$query = $db->query($sql);
...
<section class="section1">
	...
	<ul class="tab_type1">
		<li><a href="s1.php">입원환자</a></li>
		<li class="active"><a href="s1stab1.php">퇴원환자</a></li>
	</ul>
					...
					<td>
						<form method="get" name="searchForm" action="">
							<select name="searchType" class="w_input1">
								<option value="chartnum" <?=$checked=="chartnum" ? "selected" : "" ?>>차트번호</option>
								<option value="name" <?=$checked=="name" ? "selected" : "" ?>>이름</option>
								<option value="patient_bandtype" <?=$checked=="patient_bandtype" ? "selected" : "" ?>>손목형밴드</option>								
							</select>
	...
	<div class="table_wrap1">
		<table>
			...
			<tbody>
				<?php
					if($list_flag == false){
						echo "<tr><td class='txt_c' colspan='8'>퇴원인 환자가 없습니다.</td></tr>";
					}else{
				?>
				...
				<tr>
					...
					<td class="txt_c"><?=$row['bandName']?></td>
					<td class="txt_c"><?=$row['status']?></td>
					<td class="txt_c"><?=$row['regdate']?></td>
					<td class="txt_c"><a href="s1stab1sview.php?id=<?=$row['chartnum']?>&page=<?=$cur_page?>" class="bt_s1">정보수정</a></td>
				</tr>
				<?php
					$board_no--;
					}
	...
	<div class="bt_wrap1">
		<div class="left_box">
			<button type="button" class="bt_1" onclick="javascript:modiy_stat('D', 'patient')">선택삭제</button>
		</div>
	</div>
