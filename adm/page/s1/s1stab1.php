...
$count_sql = "select count(*) as cnt from tbl_management_auth where 1 ";
...
$sql = "select * from tbl_management_auth where 1 order by seq desc limit $show_offset_num, $limit_num";
$query = $db->query($sql);
...
<section class="section1">
	...
	<ul class="tab_type1">
		<li><a href="s1.php">운영자관리</a></li>
		<li class="active"><a href="s1stab1.php">권한설정</a></li>
	</ul>
					...
					<td>
						<form method="get" name="searchForm" action="">
							<select name="searchType" class="w_input1">
								<option value="name">권한</option>
								<option value="status">사용유무</option>
								<!-- <option value="">등록일</option> -->
							</select>
							<input type="text" class="w_input1" value="" name="searchKey" style="width:180px">
	...
	<div class="table_wrap1">
		<table>
			...
			<tbody>
				<?php if($list_flag == false){ ?>
				<tr><td class='txt_c' colspan="6">등록된 그룹이 없습니다.</td></tr>
				<?php }else{
				...
				<td class="txt_c"><input type="checkbox" class="rp_check_class" name="rp_check[]" value="<?=$row['seq']?>" /></td>
					...
					<td class="txt_c"><?=$row['auth_name']?></td>
					<td class="txt_c"><?=$row['delete_flag']=="1" ? "사용중지" : "사용"?></td>
					<td class="txt_c"><?=$row['regdate']?></td>
					<td class="txt_c"><a href="s1stab1sview.php?id=<?=$row['seq']?>" class="bt_s1">정보수정</a></td>
				</tr>
				<?php
						$board_no--;
					}
	...
	<div class="bt_wrap1">
		<div class="left_box">
			<button type="button" class="bt_1" onclick="javascript:modiy_stat('D', 'management_auth')">선택삭제</button>
		</div>
		<div class="right_box">
			<button type="button" class="bt_1" onclick="location.href='s1stab1swrite.php'">등록</button>
		</div>
	</div>
